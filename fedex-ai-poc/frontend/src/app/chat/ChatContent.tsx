...
function AddToDashboardModal({ chart, onClose }: { chart: ChartDataset; onClose: () => void }) {
  ...
  const handleAdd = async () => {
    const userEmail = localStorage.getItem("quantixai_user_email");
    if (!userEmail) return;
    setSaving(true);
    await fetch(`${API_BASE}/dashboard/${userEmail}/widget`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        labels: chart.labels,
        values: chart.values,
        value_label: chart.valueLabel || "Value",
        color: chart.color || "#6366f1",
        description: desc,
      }),
    });
    setSaving(false);
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
        {done ? (
          <div className="text-center py-4">
            ...
            <p className="font-semibold text-gray-900 dark:text-white mb-1">Added to dashboard!</p>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">"{chart.title}" is now on your dashboard.</p>
            <div className="flex gap-3 justify-center">
              <button onClick={onClose} className="px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700">
                Continue chatting
              </button>
              <button onClick={() => router.push("/dashboard")} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-medium">
                View dashboard →
              </button>
            </div>
          </div>
          ...
          <>
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-white">Add to Dashboard</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">"{chart.title}"</p>
              </div>
            ...
            <textarea
              ...
              rows={3}
              autoFocus
              className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-900 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none mb-4"
              placeholder="e.g. Monthly carrier volume for Q2 review…"
            />
            <button onClick={handleAdd} disabled={saving}
              className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white font-medium rounded-xl text-sm transition-colors flex items-center justify-center gap-2">
...
function DemoChartCard({ chart }: { chart: ChartDataset }) {
  ...
  return (
    <div className="rounded-xl border border-[var(--border-color)] bg-white dark:bg-slate-800 p-3 shadow-sm">
      {showAddModal && <AddToDashboardModal chart={chart} onClose={() => setShowAddModal(false)} />}
      {/* Header row */}
      ...
      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
        ...
        <button onClick={() => setShowAddModal(true)}
          className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-700 text-[11px] font-medium text-indigo-600 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors">
