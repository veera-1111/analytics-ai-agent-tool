...
const GridLayout = nextDynamic(() => import("react-grid-layout").then(m => m.Responsive || m.default), { ssr: false });
...
function buildOption(widget: Widget, type: ChartType): object {
  if (type === "bar") return {
    ...
    series: [{ data: widget.values, type: "bar", barMaxWidth: 48, itemStyle: { color: widget.color, borderRadius: [4,4,0,0] } }],
  };
  if (type === "line") return {
    tooltip: { trigger: "axis" },
    ...
    series: [{ data: widget.values, type: "line", smooth: true, symbol: "circle", symbolSize: 6,
      lineStyle: { color: widget.color, width: 2 }, itemStyle: { color: widget.color },
      areaStyle: { color: { type: "linear", x:0,y:0,x2:0,y2:1, colorStops: [{ offset: 0, color: widget.color+"44"}, { offset:1, color:"transparent" }] } } }],
  };
  return {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
      ...
      <div className="flex items-start justify-between px-4 pt-4 pb-2 gap-2 flex-wrap">
        <p className="text-sm font-semibold text-gray-900 dark:text-white leading-snug">{widget.chart_title}</p>
      ...
      {/* Download bar */}
      <div className="flex items-center gap-2 px-4 py-2.5 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-slate-900/40">
...
export default function DashboardContent() {
  ...
  const handleLayoutChange = useCallback((layouts: Layout[]) => {
    ...
    setWidgets(w => w.map(x => {
      const l = layouts.find(ll => ll.i === x.widget_id);
      if (!l) return x;
  ...
  const rglLayouts = {
    lg: widgets.map(w => ({ i: w.widget_id, x: w.layout.x, y: w.layout.y, w: w.layout.w, h: w.layout.h, minW: 3, minH: 3 })),
    md: widgets.map(w => ({ i: w.widget_id, x: w.layout.x % 8, y: w.layout.y, w: Math.min(w.layout.w, 8), h: w.layout.h, minW: 3, minH: 3 })),
  };
      ...
      <header className="sticky top-0 z-40 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-4 sm:px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {widgets.length > 0 && (
              <span className="text-xs text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full">{widgets.length} chart{widgets.length !== 1 ? "s" : ""}</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          {savingLayout && <span className="text-xs text-gray-400 animate-pulse">Saving layout…</span>}
          {!readOnly && widgets.length > 0 && (
            <button onClick={handleShare}
              ...
              {copyDone ? (
                <><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Link copied!</>
              ) : (
                <><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg>Share dashboard</>
              )}
            </button>
          )}
          {!readOnly && (
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs mb-6">
              {readOnly ? "The owner hasn't added any charts." : "Go to chat, ask a question that produces a chart, then click \"Add to Dashboard\"."}
            </p>
            {!readOnly && (
              <button onClick={() => router.push("/chat")}
          ...
          <>
            {!readOnly && (
              <p className="text-xs text-gray-400 mb-4">Drag to reposition · Drag the bottom-right corner to resize</p>
            )}
            <GridLayout
              className="layout"
              layouts={rglLayouts}
            ...
            >
              {widgets.map(widget => (
                <div key={widget.widget_id}>
                  <div className="drag-handle absolute top-0 left-0 right-0 h-8 cursor-grab active:cursor-grabbing z-10" />
                  <WidgetCard
                    widget={widget}
                    readOnly={readOnly}
